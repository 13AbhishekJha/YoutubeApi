...
# MySQL db connection.
# Use env variables in prod using docker container
...
# function for youtube in order to asist insertions to the mysql db.
...
def youtube_dialer():
    ...
    if len(response["items"]) > 0:
        ...
        return json.dumps({
            "response": "Added video data",
            "status": True
        })

    else:
        return json.dumps({
            "response": "No new videos",
            "status": True
        })